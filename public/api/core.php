...
function teleclip_read_reflect(): array {
    ...
    if (is_array($current)) {
        ...
        $history = array_values(array_filter($history, function ($item) use ($currentId) {
...
function teleclip_without_history_item(array $state, string $id): array {
    $history = is_array($state['history'] ?? null) ? $state['history'] : [];
    $history = array_values(array_filter($history, function ($item) use ($id) {
        return ($item['id'] ?? '') !== $id;
    }));
    return ['current' => $state['current'] ?? null, 'history' => $history];
}
